...
class LoginController extends Controller
{
        ...
        if ($request->isMethod('POST')) {
            ...
            if (!$validate->fails()) {
                try {
                    ...
                    if ($auth ===  1) {
                        $reject = Institute::where(['institute_email' => $email])->where('institute_status', '0')->whereNull('deleted_at')->count();

                        if ($reject === 1) {
                            return response()->json(['error' => 'You are reject.', 'errorwhy' => 'Please contact support']);
                        }
                ...
                try {
                    ...
                    if ($auth ===  1) {
                        ...
                        if (Hash::check($password, $dbpassword)) {
                            ...
                            $update = Student::where(['Email' => $email])->update([
    ...
    public function mailEnquiry(Request $request)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if ((request()->getHost() === 'www.ustudious.com')) {
